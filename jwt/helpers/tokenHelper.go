...
func UpdateAllTokens(signedToken string, signedRefreshToken string, userId string) {
	...
	updateObj = append(updateObj, bson.M{"token", signedToke})
	updateObj = append(updateObj, bson.M{"refresh_token", signedRefreshToken})

	Updated_at, _ := time.Parse(time.RFC3339, time.Now().Format(time.RFC3339))
	updateObj = append(updateObj, bson.M{"updated_at", Updated_at})
